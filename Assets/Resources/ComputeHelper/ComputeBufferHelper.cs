namespace ComputeShaderUtility
{
    ...
    public class ComputeBufferHelper
    {
        ...
        public ComputeBufferHelper(ComputeShader shader, int kernel)
        {
            _shader = shader;
            _kernel = kernel;
        ...
        public void RunShader(int x, int y, int z)
        {
            ...
            _shader.Dispatch
            (
                _kernel, Mathf.CeilToInt(x / (float) _threadsX), Mathf.CeilToInt(y / (float) _threadsY),
        ...
        {
            RWBuffer rWBuffer = new RWBuffer() {Array = reference, Buffer = new ComputeBuffer(size, stride), IsRead = true, IsWrite = false};
        ...
        {
            RWBuffer rWBuffer = new RWBuffer {Array = reference, Buffer = new ComputeBuffer(size, stride), IsRead = false, IsWrite = true};
            _buffers.Add(rWBuffer);
            _shader.SetBuffer(_kernel, name, rWBuffer.Buffer);
        }
        ...
        {
            RWBuffer rWBuffer = new RWBuffer {Array = reference, Buffer = new ComputeBuffer(size, stride), IsRead = true, IsWrite = true};
            _buffers.Add(rWBuffer);
            _shader.SetBuffer(_kernel, name, rWBuffer.Buffer);
        }
